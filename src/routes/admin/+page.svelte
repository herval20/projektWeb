<script>
  ...
  <div class="bg-black min-h-screen text-white px-6 py-8">
	<header class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
	  <h1 class="text-3xl font-bold">{$t('admin_dashboard')}</h1>
	...
	<div class="space-y-8">
	  {#each data.articles as article (article.id)}
		<div transition:slide class="bg-gray-900 rounded-xl p-6 shadow-md space-y-4">
		  ...
		  <p class="text-gray-300">ID: {article.id} - {$t('author')}: {article.author} - {article.description} - ❤️ {article.votes}</p>
  
		  <form action="?/deleteArticle" method="POST" use:enhance>
			<input type="hidden" name="id" value={article.id} />
			<button type="submit" class="bg-red-600 hover:bg-red-700 px-4 py-2 rounded-md">{$t('delete')}</button>
		  </form>
		</div>
	  {/each}
	</div>
